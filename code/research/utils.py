...
def jackknife_gradient_residuals_on_subsets(gradients: list[Tensor], subset_size: int, norm: str = "fro") -> list[float]:
    """
    Jackknife analysis of gradient residuals on subsets.
    
    Args:
        gradients: List of gradient tensors from different GPUs
        subset_size: Size of subsets to consider (k)
        norm: Type of norm to compute ("fro" for Frobenius, "nuc" for nuclear, etc.)
    
    Returns:
        List of residual norms for each subset
    """
    if not gradients or len(gradients) < subset_size:
        return []
    
    # Filter out None gradients
    valid_gradients = [g for g in gradients if g is not None]
    if len(valid_gradients) < subset_size:
        return []
    
    n_gpus = len(valid_gradients)
    
    # Compute overall mean
    overall_mean = torch.stack(valid_gradients).mean(dim=0)
    
    # Generate all combinations of subset_size from n_gpus
    combinations = list(itertools.combinations(range(n_gpus), subset_size))
    
    residual_norms = []
    
    for combo in combinations:
        # Compute subset mean
        subset_gradients = [valid_gradients[i] for i in combo]
        subset_mean = torch.stack(subset_gradients).mean(dim=0)
        
        # Compute residual and its norm
        residual = subset_mean - overall_mean
        if norm == "fro":
            residual_norm = torch.norm(residual, p='fro').item()
        elif norm == "nuc":
            residual_norm = torch.norm(residual, p='nuc').item()
        else:
            residual_norm = torch.norm(residual, p=norm).item()
        
        residual_norms.append(residual_norm)
    
    return residual_norms

# Richardson-Muon optimizer
class RichardsonMuon(torch.optim.Optimizer):
    """
    Richardson-Muon optimizer: 2 * Muon(g1...g8) - Muon(g1...g4)
    Richardson extrapolation applied to Muon optimizer for bias reduction.
    """
    def __init__(self, params, lr=0.02, weight_decay=0.01, momentum=0.95, rank=0, world_size=1):
        self.rank = rank
        self.world_size = world_size
        ...
        for group in self.param_groups:
            ...
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    state = self.state[p]
                    if len(state) == 0:
                        state["mantissa_full"] = torch.zeros_like(p, dtype=torch.uint16)
                        state["momentum_buffer_full"] = torch.zeros_like(p, dtype=torch.float32)
                        state["mantissa_half"] = torch.zeros_like(p, dtype=torch.uint16)
                        state["momentum_buffer_half"] = torch.zeros_like(p, dtype=torch.float32)
                    
                    # Collect all gradients
                    all_grads = [torch.zeros_like(p.grad) for _ in range(self.world_size)]
                    dist.all_gather(all_grads, p.grad)
                    
                    # Full gradient (all 8 GPUs)
                    grad_full = torch.stack(all_grads).mean(dim=0)
                    
                    # Half gradient (first 4 GPUs)
                    grad_half = torch.stack(all_grads[:self.world_size//2]).mean(dim=0)
                    
                    # Richardson extrapolation: 2 * Muon(full) - Muon(half)
                    # Compute Muon update for full gradient
                    momentum_buffer_full = state["momentum_buffer_full"]
                    momentum_buffer_full.copy_(momentum * momentum_buffer_full + (1 - momentum) * grad_full)
                    v_full = zeropower_via_newtonschulz5(momentum * momentum_buffer_full + (1 - momentum) * grad_full)
                    
                    # Compute Muon update for half gradient
                    momentum_buffer_half = state["momentum_buffer_half"]
                    momentum_buffer_half.copy_(momentum * momentum_buffer_half + (1 - momentum) * grad_half)
                    v_half = zeropower_via_newtonschulz5(momentum * momentum_buffer_half + (1 - momentum) * grad_half)
                    
                    # Richardson extrapolation
                    v_richardson = 2 * v_full - v_half
                    
                    # Apply update using the full gradient path's mantissa
                    acc_bf16_view_u16 = p.view(torch.uint16)
                    mantissa = state["mantissa_full"]
                    acc_m_u32 = (acc_bf16_view_u16.to(torch.uint32) << 16) | mantissa.to(torch.uint32)
                    eff_lr = torch._as_tensor_fullprec(group["lr"] * max(1, p.size(-2) / p.size(-1)) ** 0.5)
                    eff_weight_decay = torch._as_tensor_fullprec(group["lr"] * group["weight_decay"] * getattr(p, "wd_mul", 1.0))
                    
                    acc_m_u32.view(torch.float32).mul_(1 - eff_weight_decay)
                    acc_m_u32.view(torch.float32).add_(other=v_richardson, alpha=-eff_lr)
                    acc_bf16_view_u16.copy_((acc_m_u32 >> 16).to(torch.uint16))
                    mantissa.copy_(acc_m_u32.to(torch.uint16))
...
def create_model_and_optimizers(args, rank, world_size, use_richardson_muon=False):
    """Create model and optimizers with proper parameter grouping."""
    ...
    for name, param in model.named_parameters():
        if param.ndim >= 2:
            if 'attn' in name:
                attn_matrix_params.append(param)
            elif 'mlp' in name or 'fc_w' in name or 'proj_w' in name:
                mlp_matrix_params.append(param)
            else:
                other_matrix_params.append(param)
    
    # Sort by size for better efficiency
    ...
    other_matrix_params = sorted(other_matrix_params, key=lambda x: x.size(), reverse=True)
    ...
    if use_richardson_muon:
        optimizer2 = Muon(attn_matrix_params + other_matrix_params, lr=0.025, momentum=0.95, rank=rank, world_size=world_size)
        optimizer3 = RichardsonMuon(mlp_matrix_params, lr=0.025, momentum=0.95, rank=rank, world_size=world_size)
...
def run_validation(model, args, step, rank, world_size, print0, training_time_ms):
    """Run validation and return updated training time."""
    dist.barrier()
    training_time_ms += 1000 * (time.perf_counter() - time.time())  # This needs to be fixed in actual usage
    model.eval()
